...
char usage_str[] = "Usage: um_lat_pong [-h] [-a affinity_cpu] [-c config] [-E] [-g] [-p persist_mode] [-x xml_config]";
...
void help() {
  ...
  fprintf(stderr, "where:\n"
      ...
      , o_affinity_cpu, o_config, o_exit_on_eos, o_generic_src, o_persist
      , o_xml_config
  );
  CPRT_NET_CLEANUP;
  exit(0);
...
void get_my_opts(int argc, char **argv)
{
  ...
  while ((opt = getopt(argc, argv, "ha:c:Egp:x:")) != EOF) {
    switch (opt) {
      case 'h': help(); break;
...
int main(int argc, char **argv)
{
  ...
  printf("o_affinity_cpu=%d, o_config=%s, o_exit_on_eos=%d, o_persist='%s', o_xml_config=%s, \n",
      o_affinity_cpu, o_config, o_exit_on_eos, o_persist, o_xml_config);
